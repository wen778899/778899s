...
let CALC_TASK = {
    ...
    iterations: 0,
    historyCache: null,
    isProcessing: false // <--- [新增] 这是一个锁，防止重复发送
};
...
function startBot() {
    ...
    // ============================
    // 1. 后台计算任务 (已修复重复发送 Bug)
    // ============================
    setInterval(async () => {
        // 如果任务没运行，或者正在处理结算（锁住了），直接跳过
        ...
        if (isTimeUp && isIterUp) {
            ...
            try {
                ...
                if (CALC_TASK.phase === 1) {
                    ...
                    // 注意：这里不需要手动解锁，因为 phase 变了，下次进来逻辑不同
                    // 但为了保险，还是解锁，反正 phase 2 还没到时间
                ...
                // >>> Phase 2 完成 <<<
                    ...
                    CALC_TASK.isRunning = false; // 停止任务
                ...
                CALC_TASK.isProcessing = false; // 出错也要解锁，否则死锁
            }
            return;
        }
...
━━━━━━━━━━━━━━`;
        const extra = { 
            parse_mode: 'Markdown', 
            ...Markup.inlineKeyboard([Markup.button.callback('🔄 刷新', 'refresh_prog')]) 
        };
        if (isEdit) { await ctx.editMessageText(text, extra).catch(()=>{}); await ctx.answerCbQuery('状态已更新'); } 
        else { await ctx.reply(text, extra); }
    };
    ...
    bot.use(async (ctx, next) => {
        if (ctx.channelPost) { if (CHANNEL_ID && String(ctx.chat.id) === String(CHANNEL_ID)) return next(); return; }
